...
func (db *DB) ensureMigrations() error {
	...
	// columns we expect in events table
	...
	for col, typ := range expected {
		if !existing[col] {
			stmt := fmt.Sprintf("ALTER TABLE events ADD COLUMN %s %s", col, typ)
			if _, err := db.Exec(stmt); err != nil {
				log.Printf("Warning: failed to add column %s: %v", col, err)
			} else {
				log.Printf("Added missing column %s to events table", col)
			}
		}
	}
...
func (db *DB) ImportData(dataPath string) error {
	...
	for _, team := range scrapedData.Teams {
		_, err := tx.Exec(`
			INSERT INTO teams (id, name, url, logo_url)
			VALUES (?, ?, ?, ?)
		`, team.ID, team.Name, team.URL, team.LogoURL)
		if err != nil {
	...
	for _, bot := range scrapedData.Bots {
		_, err := tx.Exec(`
			INSERT INTO bots (id, name, url, rank, weight_class, points, team, team_id, team_url, description, image_url)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, bot.ID, bot.Name, bot.URL, bot.Rank, bot.WeightClass, bot.Points, bot.Team, bot.TeamID, bot.TeamURL, bot.Description, bot.ImageURL)
		if err != nil {
	...
	for _, event := range scrapedData.Events {
		...
		_, err = tx.Exec(`
		INSERT INTO events (id, name, url, start_date, end_date, location, latitude, longitude, bots_count, logo_url, description, website, organizer)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, event.ID, event.Name, event.URL, startDate, endDate, event.Location, event.Latitude, event.Longitude, event.BotsCount, event.LogoURL, event.Description, event.Website, event.Organizer)
		if err != nil {
		...
		for _, comp := range event.Competitions {
			_, err := tx.Exec(`
			INSERT INTO competitions (id, event_id, name, weight_class, url, date, begin_time, end_time, location, max_combatants, min_combatants, max_age, min_age, registration_fee)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, comp.ID, event.ID, comp.Name, comp.WeightClass, comp.URL, comp.Date, comp.BeginTime, comp.EndTime, comp.Location, comp.MaxCombatants, comp.MinCombatants, comp.MaxAge, comp.MinAge, comp.RegistrationFee)
			if err != nil {
...
func (db *DB) GetEvents(page, pageSize int, filters map[string]string) ([]models.Event, int, error) {
	query := "SELECT id, name, url, start_date, end_date, location, latitude, longitude, bots_count, logo_url, description, website, organizer FROM events WHERE 1=1"
	args := []interface{}{}
	...
	for rows.Next() {
		...
		err := rows.Scan(&event.ID, &event.Name, &event.URL, &event.StartDate, &event.EndDate, &event.Location, &event.Latitude, &event.Longitude, &event.BotsCount, &event.LogoURL, &event.Description, &event.Website, &event.Organizer)
		if err != nil {
			return nil, 0, err
		}
...
func (db *DB) GetEventByID(id string) (*models.Event, error) {
	var event models.Event
	err := db.QueryRow(`
		SELECT id, name, url, start_date, end_date, location, latitude, longitude, bots_count, logo_url, description, website, organizer
		FROM events WHERE id = ?
	`, id).Scan(&event.ID, &event.Name, &event.URL, &event.StartDate, &event.EndDate, &event.Location, &event.Latitude, &event.Longitude, &event.BotsCount, &event.LogoURL, &event.Description, &event.Website, &event.Organizer)
...
func (db *DB) GetTeams(page, pageSize int) ([]models.Team, int, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&team.ID, &team.Name, &team.URL, &team.LogoURL)
		if err != nil {
			return nil, 0, err
		}
...
func (db *DB) GetTeamByID(id string) (*models.Team, error) {
	...
	err := db.QueryRow(`
		SELECT id, name, url, logo_url
		FROM teams WHERE id = ?
	`, id).Scan(&team.ID, &team.Name, &team.URL, &team.LogoURL)
